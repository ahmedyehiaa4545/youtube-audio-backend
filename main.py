...
def call_openrouter_shorts(transcription: str, num_shorts: int, api_key: str, model_name: str = "google/gemini-3.5-flash-lite", custom_prompt: str = None, title_style: str = "auto"):
    ...
    system_prompt = (
        "أنت خبير ومخرج محترف في صناعة المحتوى الفيروسي الأكثر مشاهدة (Viral Content Creator & Growth Hacker).\n"
        "مهمتك الرئيسية: استخراج عناوين مثيرة جداً، حماسية، جريئة، وتثير الفضول الرهيب (Curiosity Gap & Suspense) لتجبر المشاهد على النقر وإكمال الفيديو لنهايته.\n"
        "يجب أن تكون إجابتك بصيغة JSON فقط بالتنسيق التالي بدون أي نصوص إضافية خارج الـ JSON:\n"
        "{\n"
        '  "shorts": [\n'
        '    {\n'
        '      "title": "عنوان جريء ومثير جداً يثير الفضول ويجبر المشاهد على إكمال القصة",\n'
        '      "start_time": "05:47",\n'
        '      "end_time": "06:20",\n'
    ...
    title_instruction = (
        "5. صياغة عنوان فيروسي، حماسي، وجريء جداً يثير فضولاً قاتلاً ولا يُقاوم (Curiosity Gap & Suspense):\n"
        "   - إياك ثم إياك العناوين العادية أو الملخصة أو الملل المباشر.\n"
        "   - اختر أجرأ أو أغرب أو أكثر نقطة تشويقية وصادمة داخل الأحداث واجعلها محور العنوان (مثال: 'اللحظة التي تغير فيها كل شيء بنفس الدقيقة...', 'كيف اكتشف الحقيقة الصادمة قبل فوات الأوان؟', 'السر الذي حاولوا إخفاءه طوال السنوات!').\n"
        "   - اجعل من يقرأ العنوان يشعر برغبة ملحة وفضول جارف لنقر الفيديو وإكمال المشاهدة حتى النهاية ليعرف بقية الحكاية بنفسه."
    )
    if title_style == "short":
        title_instruction = (
            "5. صياغة عنوان فيروسي جريء وقصير جداً من 2 إلى 4 كلمات فقط (حد أقصى 5 كلمات) يثير الفضول والتشويق القاتل فوراً."
        )
    elif title_style == "medium":
        title_instruction = (
            "5. صياغة عنوان فيروسي حماسي وجريء من 5 إلى 9 كلمات، يلتقط الجزء الأكثر إثارة من القصة ويجعل المشاهد يكمل الفيديو للنهاية."
        )
...
def cut_video(req: CutRequest):
    ...
    ytdl_cmd = [
        ...
        '--downloader-args', 'ffmpeg:-c:v libx264 -preset ultrafast',
        '-o', temp_raw_path
    ]
